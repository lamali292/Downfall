...
public sealed class PlayerField<TVal>
{
    ...
    public PlayerField(Func<TVal?> defaultVal) => _inner = new SpireField<PlayerCombatState, TVal>(defaultVal);

    public PlayerField(Func<PlayerCombatState, TVal?> defaultVal) =>
        _inner = new SpireField<PlayerCombatState, TVal>(defaultVal);

    public TVal? Get(Player? obj) => Get(obj?.PlayerCombatState);

    public void Set(Player? obj, TVal? val) => Set(obj?.PlayerCombatState, val);

    public TVal? Get(PlayerCombatState? obj) => obj is null ? default : _inner.Get(obj);
