...
{
    private readonly List<(CharacterModel left, CharacterModel right)> _pairs = new();
    private readonly List<CharacterModel> _chosen = new();
    ...
    {
        if (Owner!.Character is not Core.Snecko)
    ...
    private IReadOnlyList<EventOption> BuildPage(int page)
    {
        ...
        {
            CharOption(left,  page),
            CharOption(right, page),
        };
    }

    // static loc-key options (DollRoom pattern): title/description come from
    // {slug}.pages.PAGE_{page}.options.{LEFT|RIGHT}.title/.description — no runtime character member access
   /* private EventOption CharOption(CharacterModel c, int page, string slot) =>
        new(this,
            () => OnPicked(c, page),
            $"{StringHelper.Slugify(GetType().Name)}.pages.PAGE_{page}.options.{slot}",
   ...
   public readonly Dictionary<EventOption, CharacterModel> OptionCharacters = new();
   ...
   private EventOption CharOption(CharacterModel c, int page)
   {
    ...
    // ---- selection flow ----
    private Task OnPicked(CharacterModel c, int page)
    {
        _chosen.Add(c);

        if (page + 1 < _pairs.Count)
        {
            // SetEventState fires StateChanged -> patched RefreshEventState reads CurrentTranscriptLines
            SetEventState(
        ...
        {
            //SneckoModel.SetSneckoPools(Owner!, _chosen.Select(x => x.CardPool));
            SetEventFinished(L10NLookup($"{Id.Entry}.pages.DONE.description"));

            // hand off to Neow — valid only because its node hasn't been entered yet (act-0 redirect)
            TaskHelper.RunSafely(
                RunManager.Instance.EnterMapCoord(Owner!.RunState.Map.StartingMapPoint.coord));
        }
    ...
    protected override AncientDialogueSet DefineDialogues()
    {
        ...
        {
            FirstVisitEverDialogue = new AncientDialogue("event:/sfx/npcs/snecko_spirit/hiss")  // your voice
            {
                // if VisitIndex/other props are needed, mirror what base set
            },
    ...
    private IReadOnlyList<AncientDialogueLine> BuildTranscript()
    {
        ...
        for (var page = 0; page <= _chosen.Count && page < 3; page++)
        {
            ...
            if (page < _chosen.Count)
                AppendDialogue(lines, "ECHO", page, _chosen[page]);   // fixed key, character passed separately
        }
        return lines;
    }
